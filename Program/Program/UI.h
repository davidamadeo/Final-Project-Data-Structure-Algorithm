...
void UI::readFromTransactionTXT(){
	...
	//IF IT IS EXIST
		...
		while(file){
			...
			//SEPARATES EACH STRING AND INSERT THEM TO REAL TRANSACTIONHISTORY LIST
...
//FUNCTION TO CHECH THE EXIST OF HOLDER
...
//FUNCTION TO CHECH THE EXIST OF "ONLY" SAVING HOLDER
...
//FUNCTION TO CHECH THE EXIST OF "ONLY" CREDIT HOLDER
...
bool UI::isNegative(int amount){
	if (amount < 0 ){
		cout<<"Amount has to be positive"<<endl;
		return true;
	}
	else{
		return false;
...
void UI::addHolder(){
	...
	//INSERT HOLDER DEPENDS ON THE TYPE
	if(type == "1"){
...
void UI::showTransactionHistory(){
	...
	cout<<	"*LEGEND\n"
			...
			"NEW = INITIALIZE SAVING OR CREDIT"<<endl;
}
...
void UI::payment(){
	...
	NODE<Holder>* curr = holderList.getFirst();
	if (curr == NULL){
		cout<<"You've got no Holder, Please create a new one"<<endl;
	}
	else{
		//Choose Holder with validation
		showSavingHolders();
		showCreditHolders();
		int num = 0;
		do{
...
void UI::transferToOtherHolder(){
	...
	//if there are then...
		...
		//checks whether the sender holder exist
		showSavingHolders();
		string sender;
		int num = 0;
		do{
		...
		//checks for the exist of receiver holder
		string receiver;
		int num2 = 0;
		do{
			if (num2 != 0){
		...
		//adds balance of the receiver and subtract balance of the sender
		NODE<Holder>* curr = holderList.getFirst();
		NODE<Holder>* curr2 = holderList.getFirst();
	
		while(curr!=NULL){
			//checks whether the sender balance is sufficient and subtract the balance of the sender
...
void UI::payDueCredit(){
	payDueCreditHeader();
	//checks whether there is atleast 1 credit holder
	...
	//if there are...
		...
		//checks for the exist receiver holder
		showCreditHolders();
		string receiver;
		int num2 = 0;
		do{
		...
		//checks for the exist of source holder
		showSavingHolders();
		string sender;
		int num = 0;
		do{
		...
		//if there is no due credit in the choosen holder
		if (amount == 0){
